...
FancyInput = {
 started: false,
 tlist: null,
 init: function(input, auto) {
   if (!this.started) {
     this.tlist = new FacebookList(input, auto, {newValues: true});
     // fetch and feed
     new Ajax.Request($(auto).readAttribute('target'), {
       method: 'get',
       onSuccess: function(transport) {
           transport.responseText.evalJSON(true).each(function(t){this.tlist.autoFeed(t)});
       }
     });
     this.started = true;
   }
...
var FacebookList = Class.create(TextboxList, { 
  ...
  autoAdd: function(el) {
	...
	// FIXME: This is a bad hack - copies and pastes all over autoAdd...
	addBeforeSubmit: function() {
		try {
			var el = $$('input.maininput').last();
		  el.value = el.value.strip().gsub(",","");
		  if(!this.options.get("spaceReplace").blank()) 
				el.gsub(" ", this.options.get("spaceReplace"));
		  if(!el.value.blank()) {
		    this.newvalue = true;
		    if(this.newvalue && this.options.get("newValues")) {
		      this.add({caption: el.value, value: el.value, newValue: true});
        ...
        switch(e.keyCode) {
          ...
          case Event.KEY_RETURN:
            ...
            if(this.autocurrent && this.resultsshown) { 
							// is a autocomplete area, select the selected item
